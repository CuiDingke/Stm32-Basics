...
float Information,informa,L;	
char A[50],B[
	50],C[50],D[50];
extern uint8_t p,q;
uint8_t  i=0,j=0,k=1,G=1;
int main(void)
{
	...
	while(1)
	{
		IIC_WriteOneData(4,a);
    IIC_WriteOneData(5,b);
	  IIC_WriteOneData(6,c);
		...
		if(k==0&&p==1)
{
	...
	k=1;
      ...
      Information=ADC_ConvertedValue[0]*3.3/4096;
		  sprintf (B,"   ADC    :%.2fV   ",Information);
      LCD_DisplayStringLine(Line5,(unsigned char* )B);

		  informa=ADC_ConvertedValue[0]*101/4096;
		  sprintf (C,"   Height :%.0fcm    ",informa);
      LCD_DisplayStringLine(Line3,(unsigned char* )C);
      i=0;
			
    if(informa<=10)
    {L=0;
      sprintf (A,"   Level  :%.0f",L);
      LCD_DisplayStringLine(Line7,(unsigned char* )A);
      Delay_Ms(1000);
    }
    if(informa>10&&informa<=20)0
    {L=1;
      sprintf (A,"   Level  :%.0f     ",L);
      LCD_DisplayStringLine(Line7,(unsigned char* )A);
      Delay_Ms	(1000);
   ...
   {L=3;
    ...
    Delay_Ms	(1000);
	  ...
	  IIC_WriteOneData(1,Information);
    IIC_WriteOneData(2,informa);
	  IIC_WriteOneData(3,L);
   }
